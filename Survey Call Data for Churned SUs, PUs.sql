/*
- Viz: 
- Data: https://docs.google.com/spreadsheets/d/1cfP1Y4fuSiCymqrwsmlWRqYz6jGhZSgH1mpPvF1XO5g/edit#gid=1268800390 
- Function: 
- Table:
- Instructions: 
- Format: 
...
select mobile_no, max(tg) tg_before_eid
from 
	(select mobile mobile_no
	from tallykhata.tallykhata_clients_location_info
	where district_name in('Chittagong', 'Dhaka', 'Gazipur', 'Narayanganj')
	) tbl1 
	...
	where 
		tg in('PUCb','PU Set-A','PU Set-B','PU Set-C','PUTa','PUTa+Cb','PUTacs','SPU') 
		and report_date>='28-Apr-22'::date and report_date<='02-May-22'::date 
	...
	where date(created_at)<current_date-(7*25)
	...
	where event_date>='08-May-22'::date
...
where 
	biz_type in('GROCERY', 'MFS-MOBILE RECHARGE STORE')
	or biz_type is null;

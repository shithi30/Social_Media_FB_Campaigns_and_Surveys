...
-- active last 7 days
drop table if exists data_vajapora.temp_e;
create table data_vajapora.temp_e as
...
from 
	(select 
		*, 
		case 
			when district_name='Madaripur' then data_vajapora.lat_long_dist_meters(23.1649, 90.1940, lat::numeric, lng::numeric) 
			when district_name='Shariatpur' then data_vajapora.lat_long_dist_meters(23.2423, 90.4348, lat::numeric, lng::numeric) 
		end distance, 
	...
	from 
		...
		where 
			district_name in('Madaripur', 'Shariatpur')
			...
			and 
				(data_vajapora.lat_long_dist_meters(23.1649, 90.1940, lat::numeric, lng::numeric)<=5000 -- Madaripur
				or 
				data_vajapora.lat_long_dist_meters(23.2423, 90.4348, lat::numeric, lng::numeric)<=50000 -- Shariatpur
				)			
		) tbl1 
		inner join 
		data_vajapora.temp_b tbl2 using(mobile_no)
		inner join 
		data_vajapora.temp_c tbl3 using(mobile_no)
		inner join 
		data_vajapora.temp_d tbl4 using(mobile_no)
		inner join 
		data_vajapora.temp_e tbl5 using(mobile_no)
	) tbl1
